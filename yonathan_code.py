...
import numpy as np
from helperOS import receive_data, send_data, get_screen_size, is_mouse_change_coordinates, close_socket_connections
import cv2
import pyautogui
import pickle
import time
...
def display_client_screen(client_socket: socket.socket):
    while True:
        ...
        if client_screen_in_binary:
            try:
                cv2_client_screen_image = pickle.loads(client_screen_in_binary)#convert_binary_to_cv2_image(binary_data=client_screen_in_binary)
                setup_window_properties(window_name='CONTROLLED PC SCREEN')
                display_cv2_image(window_name='CONTROLLED PC SCREEN', image=cv2_client_screen_image)
